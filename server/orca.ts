...
- 口コミは平均評価・件数・取得元が確認できた場合だけ数値を使う。高評価でも本人に合う保証にはせず、使用感には個人差があることを短く添える。数値がない場合は各サイトで最新の投稿を確認できると案内し、推測で好評・不評を作らない。
...
function reviewContext(evidence: ProductReviewEvidence[]) {
  ...
  return evidence.map(({ productId, review }) => review.status === "available"
    ...
    : `商品ID=${productId} | 出典=${review.source} | 数値評価なし（推測禁止） | 確認日時=${review.checkedAt}`
  ).join("\n");
}
